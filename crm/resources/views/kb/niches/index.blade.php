<x-app-layout>
    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <p class="mb-4 text-sm text-gray-500 dark:text-gray-400">Медицинские направления: онкология, кардиология, ЭКО и др.</p>
                    ...
                    <tbody>
                        @forelse($niches as $niche)
                            <tr class="border-b border-gray-100 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition">
                                <td class="py-2 pr-3 font-mono text-xs text-gray-500 dark:text-gray-400">{{ $niche->code }}</td>
                                <td class="py-2 pr-3">
                                    <a href="{{ route('niches.show', $niche) }}" class="text-indigo-600 dark:text-indigo-400 hover:underline font-medium">
                                        {{ $niche->name }}
                                    </a>
                                </td>
                                <td class="py-2 text-gray-600 dark:text-gray-400">{{ $niche->description ?? '—' }}</td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="3" class="py-8 text-center text-gray-400 dark:text-gray-500 italic">
                                    Ниши ещё не добавлены
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
                <div class="mt-4">{{ $niches->links() }}</div>
            </div>
        </div>
    </div>
</x-app-layout>

<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            <a href="{{ route('kb.message-templates.index') }}" class="text-dc-secondary hover:text-dc text-ys-s dc-transition">← Шаблоны</a>
            <h2 class="text-ys-l font-semibold text-dc leading-tight">Новый шаблон</h2>
        </div>
    </x-slot>
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                            <div>
                                ...
                                @error('channel')<p class="text-dc-red-100 text-ys-xs mt-1">{{ $message }}</p>@enderror
                            </div>
                            <div>
                                ...
                                @error('language')<p class="text-dc-red-100 text-ys-xs mt-1">{{ $message }}</p>@enderror
                            </div>
                            <div>
                                ...
                                <select name="target_partner_type" class="block w-full h-9 px-3 text-ys-s rounded-2xs border border-dc-gray-30 bg-surface text-dc dc-transition focus:outline-none focus-visible:ring-2 focus-visible:ring-dc-yellow-100">
                                    ...
                                    <option value="clinic" @selected(old('target_partner_type') === 'clinic')>Клиника</option>
                                    <option value="translator" @selected(old('target_partner_type') === 'translator')>Переводчик</option>
                                    <option value="curator" @selected(old('target_partner_type') === 'curator')>Куратор</option>
                                </select>
                            </div>
                        </div>
                        <div>
                            <label class="block text-ys-xs font-medium text-dc-secondary mb-1">Тема письма</label>
                            <input type="text" name="subject" value="{{ old('subject') }}" class="block w-full h-9 px-4 text-ys-s rounded-2xs border border-dc-gray-30 bg-surface text-dc dc-transition focus:outline-none focus-visible:ring-2 focus-visible:ring-dc-yellow-100">
                        </div>
                        <div>
                            <label class="block text-ys-xs font-medium text-dc-secondary mb-1">Текст сообщения <span class="text-dc-red-100">*</span></label>
                            <textarea name="body" rows="8" required class="block w-full p-3 text-ys-s rounded-2xs border border-dc-gray-30 bg-surface text-dc dc-transition focus:outline-none focus-visible:ring-2 focus-visible:ring-dc-yellow-100 font-mono">{{ old('body') }}</textarea>

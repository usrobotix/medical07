        ...
        <div class="flex items-center justify-between">
            ...
            @auth
                @if(auth()->user()->hasAnyRole(['admin', 'manager']))
                    <a href="{{ route('kb.country-directions.create') }}" class="px-4 py-2 text-sm bg-indigo-600 text-white rounded-md hover:bg-indigo-700 transition font-medium">+ Добавить</a>
                @endif
            @endauth
        </div>
    </x-slot>
            ...
            <div class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg p-4">
                <form method="GET" action="{{ route('kb.country-directions.index') }}" class="flex flex-wrap gap-3 items-end">
                    ...
                    <div class="flex gap-2">
                        <x-kb-apply-button />
                        <x-kb-reset-link :href="route('kb.country-directions.index')" />
                    </div>
                </form>
            </div>

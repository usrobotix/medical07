...
## Где хранятся данные в CRM

В репозитории medical07 датасет хранится по пути:

```
crm/storage/app/research/Research/
```

Эта директория включена в git (исключение прописано в `.gitignore` через `storage/app/.gitignore`).

При первоначальной установке (или если данные отсутствуют) нужно скопировать датасет из репозитория yandex-direct:

```bash
# Вариант 1: копировать из локального клона yandex-direct
cp -r /path/to/yandex-direct/Research crm/storage/app/research/

# Вариант 2: клонировать yandex-direct и скопировать
...
```bash
# После git pull (если обновились данные или код)
cd crm
git pull origin main
php artisan optimize:clear
php artisan migrate   # только если были новые миграции
php artisan research:import-partners  # обновить данные клиник
```

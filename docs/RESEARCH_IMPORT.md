...
Репозиторий включает **один образец-клиника** для демонстрации и тестирования `--dry-run`:
...
Полный датасет (350 клиник) **не входит в репозиторий** из-за ограничений Windows (см. раздел ⚠️ выше). После клонирования/pull нужно скопировать папку `Research/` вручную:
...
- Ключевые услуги
- Список источников
- Текст review.md — отображается как **отформатированный HTML** (Markdown-рендеринг)
...
- Laravel 10+
- `league/commonmark` ^2.4 (для рендеринга Markdown в UI — добавлен в `composer.json`)
